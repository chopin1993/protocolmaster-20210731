...
config["应用程序上一版发布版本"] = "P-SN-IR-IM-A(v1.0)-20190308"  # 如果此处为空字符串时，表示不支持兼容性升级或手动测试。
config["应用程序同版本号测试版本"] = "P-SN-IR-IM-A(v1.1)-20200728"
...
def test_兼容性升级测试():
    ...
    engine.add_doc_info('人体红外感应器新旧版本功能一致，所有的参数均可以进行正常验证')
    # 升级后再次查看版本和配置信息
