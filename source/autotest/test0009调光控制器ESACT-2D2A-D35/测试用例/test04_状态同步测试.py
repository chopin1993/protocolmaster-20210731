...
def test_添加上报():    # finished
    ...
    set_gw_info()  # 模拟设备入网
    passed_time = read_write_test()  # 配置入网后的读写测试
    engine.wait((15 - passed_time), allowed_message=False)
...
def test_上电上报():    # finished
    """
    02_上电上报测试
    上电上报时间计算公式：延时时间60s（网关重启组网时间）+ rand 秒，其中rand=sid% 100。
    ...
    set_gw_info(panid=1100, sid=120)    # sid为120，注意配置对应的上报等待时间
    engine.wait(14, allowed_message=False)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", '00', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], ack=True)
    report_power_on_expect(wait_times=[80], ack=True)   # 上电上报期待

    engine.add_doc_info("2、测试sid < 100的情况，sid = 8的情况下，测试网关正常应答的情况")
    report_gateway_expect(wait_times=[15.5], ack=True)
    ...
    set_gw_info(panid=1112, sid=11)
    # 前端工装断电重启，模拟上电上报
    power_control()     # 控制设备通断电
    engine.add_doc_info('上电上报之前设备无上报')
    start_time = time.time()
    for i in range(2):
        engine.send_did("WRITE", "继电器翻转C018", "81")
        engine.expect_did("WRITE", "通断操作C012", "**")
        engine.wait(3, tips='控制继电器完全翻转，等待')

    engine.add_doc_info('此处等待69s')
    passed_time_1 = time.time() - start_time
    engine.wait(54-passed_time_1, allowed_message=False)
    engine.add_doc_info('上电上报回复')
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", '00', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], ack=True, timeout=2)  # 上电上报，需要回复
    # 设备通断状态设置回断开
    if engine.expect_did == {'WRITE', '通断操作C012', '01'}:
        engine.send_did("WRITE", "通断操作C012", "01")
        engine.expect_did("WRITE", "通断操作C012", "00")
        engine.send_did("WRITE", "通断操作C012", "02")
        engine.expect_did("WRITE", "通断操作C012", "00")
        engine.expect_multi_dids("REPORT",
                                 "通断操作C012", '00', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **", ack=True)
        engine.expect_multi_dids("REPORT",
                                 "通断操作C012", '00', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                                 "导致状态改变的控制设备AIDC01A", config["测试设备地址"], ack=True, timeout=5)
    else:
        engine.add_doc_info('**********当前状态是关闭状态，不需要进行恢复操作**********')
...
def test_配置订阅者():   # ing
    ...
    # report_power_on_expect(wait_times=[71], ack=True, wait_enable=False)
    # report_subscribe_expect(devices=[], write_value="81", expect_value="01", scene_type="网关单点控制")
    # report_subscribe_expect(devices=[], write_value="01", expect_value="00", scene_type="网关单点控制")

    # engine.add_doc_info("2、配置1个订阅者：使用网关控制后，立即回复网关，然后按顺序上报订阅者")
    # report_power_on_expect(wait_times=[74], ack=True, wait_enable=True)
    ...
    # engine.expect_did("WRITE", "通断操作C012", "00")
    # engine.wait(10, allowed_message=False)
    engine.wait(120, tips='等待是否有多余的未处理报文')
    # 恢复设备状态
    back_devices()
...
def test_过温上报():
    ...
    1.配置设备入网。
    2.设置过温温度C0A5（模拟过温）
    （1）设置为0X64-100度-->开灯-->检测上报
    （2）设置为0x20-32度（单通道）-->过温上报无回复-->10s，100s重发检测
    （3）设置为0x64--100度恢复正常-->控制亮度-->控制后应无过温上报
    （4）设置为0x20-32度（双通道）
    :return:无
    """
    engine.report_check_enable_all(True)

    engine.add_doc_info('测试步骤1：过温上报--配置设备入网')
    set_gw_info(panid=1111, sid=10)  # 1.设备入网
    engine.wait(14, allowed_message=False)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", '00', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], ack=True)
    report_power_on_expect(wait_times=[70], ack=True, wait_enable=False)  # 上电上报期待

    engine.add_doc_info('测试步骤2：过温上报--设置为0X64-100度')
    ...
    # 由于上面配置了，正常的温度（模拟），下面的控制不应该触发过温上报 C0A0
    engine.add_doc_info('测试步骤3：过温上报--开灯')
    engine.send_did('WRITE', '亮度值C0A2', '01 0a')
    engine.expect_did('WRITE', '亮度值C0A2', '01 0a **')
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '01', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["抄控器默认源地址"], ack=True)
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '01', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["抄控器默认源地址"], ack=True, timeout=5)

    engine.add_doc_info('测试步骤4：过温上报--设置为0x05-5度（单通道）')
    engine.send_did('WRITE', '查询设置超温保护温度上限C0A5', '01 05')
    engine.expect_did('WRITE', '查询设置超温保护温度上限C0A5', '01 05')
    engine.expect_did('REPORT', '超过保护温度关灯并上报C0A0', '01', ack=True)
    engine.add_doc_info('过温后关灯，C01A应该是自身')
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", '00 00 00 00', ack=True)  # 开灯调整到最高亮度，等待灯具升温，是否有过温上报;无过温上报合格

    engine.add_doc_info('测试节点5：超温C0A0上报网关，无回复重发机制')
    engine.add_doc_info('调节亮度，触发过温上报')
    engine.send_did('WRITE', '亮度值C0A2', '01 11')
    engine.expect_did('WRITE', '亮度值C0A2', '0F 00')
    engine.add_doc_info('期待过温上报C0A0')
    engine.expect_did('REPORT', '超过保护温度关灯并上报C0A0', '01', ack=False, timeout=5)  # 不给与回复，期待重发
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", '00 00 00 00', ack=True)
    engine.expect_multi_dids('REPORT', '超过保护温度关灯并上报C0A0', '01', ack=False, timeout=10.5)  # 10s上报网关，超温
    engine.expect_multi_dids('REPORT', '超过保护温度关灯并上报C0A0', '01', ack=False, timeout=100.5)  # 100s上报重发检测

    engine.add_doc_info('测试步骤6：1通道过温后，测试2通道控制是否正常')
    engine.send_did('WRITE', '亮度值C0A2', '02 32')
    engine.expect_did('WRITE', '亮度值C0A2', '02 32 **')
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["抄控器默认源地址"], ack=True)
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", '00 00 00 00', ack=True, timeout=5)

    engine.add_doc_info('测试步骤7:过温上报--设置为0x64--100度，使其恢复正常')
    engine.send_did('WRITE', '查询设置超温保护温度上限C0A5', '01 64')
    engine.expect_did('WRITE', '查询设置超温保护温度上限C0A5', '01 64')
    engine.wait(1)
    engine.send_did("WRITE", "亮度值C0A2", "01 32")
    engine.expect_did("WRITE", "亮度值C0A2", "01 ** **")
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["抄控器默认源地址"], ack=True)
    engine.expect_multi_dids('REPORT',
                             "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                             "导致状态改变的控制设备AIDC01A", config["抄控器默认源地址"], ack=True, timeout=5)
    engine.wait(10, allowed_message=False)  # 正常情况下控制，应无异常上报，等待查询是否有异常

    engine.add_doc_info('测试步骤8:设置为0x11-17度（双通道）')
    for i in range(1, 3):  # 1,2通道过温
        if i == 1:
            engine.send_did('WRITE', '查询设置超温保护温度上限C0A5', '01 05')
            engine.expect_did('WRITE', '查询设置超温保护温度上限C0A5', '01 05')
            engine.expect_did('REPORT', '超过保护温度关灯并上报C0A0', '01', ack=True)
            engine.add_doc_info('通道1-过温后关灯上报，C01A应该是自身00 00 00 00')
            engine.expect_multi_dids('REPORT',
                                     "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                                     "导致状态改变的控制设备AIDC01A", '00 00 00 00', ack=True)
        else:
            engine.send_did('WRITE', '查询设置超温保护温度上限C0A5', '02 05')
            engine.expect_did('WRITE', '查询设置超温保护温度上限C0A5', '02 05')
            engine.wait(0.5)
            engine.add_doc_info('通道2-过温后关灯上报，C01A应该是自身00 00 00 00')
            engine.expect_multi_dids('REPORT',
                                     "通断操作C012", '**', "亮度值C0A2", "01 ** **", "亮度值C0A2", "02 ** **",
                                     "导致状态改变的控制设备AIDC01A", '00 00 00 00', ack=True)
            engine.expect_multi_dids('REPORT', '超过保护温度关灯并上报C0A0', '01',  '超过保护温度关灯并上报C0A0', '02', ack=True)

    engine.add_doc_info('--------------the test is finished !!!--------------')
    engine.add_doc_info('配置设备恢复正常状态')
    back_devices()  # 恢复设备
    engine.wait(120, tips='等待，查看是否有其他报文')
...
def test_保险丝上报():
    ...
    :return:
    """
    engine.add_doc_info('！！！注：需要人工断开保险丝模拟故障，此处做手动补充测试')
