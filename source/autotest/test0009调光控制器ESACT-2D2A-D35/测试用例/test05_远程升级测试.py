...
config["应用程序上一版发布版本"] = "ESACT-2D2A-D35(v1.1)-20191225"  # 如果此处为空字符串时，表示不支持兼容性升级或手动测试。
config["应用程序同版本号测试版本"] = "ESACT-2D2A-D35(v1.2)-20210312"
...
# 特别版本
config["载波适配层上一版本"] = "ESMD-AD63(v2.2)-20170826"
config["载波网络层上一版本"] = "SSC1663-PLC(v1.0)-20171011"
...
def init_升级测试环境搭建():    # 通过测试20210318-14-04
    """
    00_测试环境搭建
...
def test_兼容性升级测试():     # 测试通过
    ...
    engine.expect_multi_dids("READ",
                             ...
                             "DKEY0005", config["DKEY0005"],
                             "SN0007", config["SN0007"])
    engine.add_doc_info("测试步骤说明：兼容性升级前读取默认参数")
    engine.send_did("READ", "线性调光时间C0A3", "01")
    engine.expect_did("READ", "线性调光时间C0A3", "01 05")
    engine.send_did("READ", "线性调光时间C0A3", "02")
    engine.expect_did("READ", "线性调光时间C0A3", "02 05")
...
def test_升级结束后恢复默认参数():
    ...
    10_升级结束后恢复默认参数
    """
    engine.add_doc_info('测试步骤说明：设置-恢复默认参数，便于后续的测试项目运行')
