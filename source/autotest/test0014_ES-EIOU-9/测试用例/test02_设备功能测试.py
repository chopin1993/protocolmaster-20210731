...
def test_模式与数据测试():
    ...
    engine.add_doc_info('1.2 读取所有通道（00）状态')
    ...
    for channel in range(1, 10):
        # 循环配置设备9个通道状态
        engine.send_FE02_did("WRITE", "IO配置D201", '0'+str(channel)+" 00")
        engine.expect_FE02_did("WRITE", "IO配置D201", '0'+str(channel)+" 00")
...
def test_NTC_PTC_input():
    ...
    # 0x18--k24--io-4--220k
    # 0x19--k25--io-5--2m
    engine.add_doc_info('15-控制工装200R输出,读取通道测温数值')
    ctrl_relay_open(15)     # 0x15继电器 工装200R欧姆输出
    engine.wait(seconds=1, tips='等待1s，200R接通')

    # engine.send_FE02_did('READ', 'IO操作D200', '01 06 07 08 09')
    # engine.expect_FE02_did('READ', 'IO操作D200', '01 20 02 FF FF FF FF 06 20 02 FF FF FF FF 07 20 02 '
    #                                            'FF FF FF FF 08 20 02 FF FF FF FF 09 20 02 FF FF FF FF')
    # engine.send_FE02_did('READ', 'IO操作D200', '02 03 04 05')
    # engine.expect_FE02_did('READ', 'IO操作D200', '01 20 02 FF FF FF FF 06 20 02 FF FF FF FF 07 20 02 '
    #                                            'FF FF FF FF 08 20 02 FF FF FF FF 09 20 02 FF FF FF FF')
