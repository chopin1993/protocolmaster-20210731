...
def test_出厂默认参数():
    ...
    7、继电器版继电器动作次数读取；
    """
    engine.send_did('READ', '过压、欠压跳闸阈值', '')
    engine.send_did('READ', '过压、欠压跳闸阈值', '00 40 26 60 17')
...
def test_继电器动作次数C132():
    ...
    if config["被测设备硬件版本"] == "继电器版本":
        ...
        for channel, value in channel_dict.items():
            ...
            expect = value + " " + str(hex(int(cnt/256))[2:]).rjust(2, "0") + " " + str(hex(int(cnt%256))[2:]).rjust(2, "0")
