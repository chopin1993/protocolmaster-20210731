...
def test_添加上报():
    ...
    # engine.wait(14.5, expect_no_message=False)
    ...
    set_gw_info()  # 设置网关PANID信息，模拟设备入网
    engine.wait(14.5, expect_no_message=True)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", "**",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], timeout=1)
    engine.wait(9.5, expect_no_message=True)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", "**",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], timeout=1)
    engine.wait(99.5, expect_no_message=True)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", "**",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], timeout=1)
    engine.wait(20, expect_no_message=True)
    engine.send_did("WRITE", "退网通知060B", 退网设备=config["测试设备地址"])
    ...
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", "**",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], timeout=1)
    engine.wait(120, expect_no_message=True)
...
def test_上电上报():
    ...
    engine.add_doc_info("测试测试sid > 100的情况，sid = 120的情况下，测试网关正常应答的情况")
    config = engine.get_config()
    engine.send_local_msg("设置PANID", config["panid"])
    engine.expect_local_msg("确认")
    engine.send_did("WRITE", "载波芯片注册信息0603",
    ...
    engine.wait(79, expect_no_message=True)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", "**",
    ...
    engine.wait(67, expect_no_message=True)
    engine.expect_multi_dids("REPORT",
                             "通断操作C012", "**",
                             "导致状态改变的控制设备AIDC01A", config["测试设备地址"], timeout=2, ack=True)  # 预留1s的误差
...
def test_网关无应答时设备上报的重试机制():
    ...
    panel03.expect_did("NOTIFY", "通断操作C012", "01", timeout=2)
    engine.expect_multi_dids("REPORT", "通断操作C012", "01", "导致状态改变的控制设备AIDC01A", panel01.src, timeout=1)
    engine.wait(5, expect_no_message=True)
