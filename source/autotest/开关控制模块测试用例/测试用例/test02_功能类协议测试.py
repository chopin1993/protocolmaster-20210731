...
def test_通断操作C012():
    ...
    engine.wait(1, expect_no_message=False)
    engine.send_did("READ", "通断操作C012")
...
def test_继电器上电状态C060():
    ...
    # 00上电状态为上电断开
    engine.add_doc_info("测试上电状态为上电断开，通过前置的大功率开关通断，给开关控制模块通断电")
...
def test_继电器过零点动作延迟时间C020():
    ...
    engine.add_doc_info("测试开关控制模块断电重启，要求配置参数前后保持不变")
    power_off_test()
    engine.send_did("READ", "继电器过零点动作延迟时间C020", "01")
    engine.expect_did("READ", "继电器过零点动作延迟时间C020", "01 20 20")
...
def test_设备运行状态信息统计E019():
    ...
    engine.send_did("WRITE", "设备运行状态信息统计E019", E019设备信息项="延时关闭时间毫秒", 时间=60)
    engine.expect_did("WRITE", "设备运行状态信息统计E019", E019设备信息项="延时关闭时间毫秒", 时间=60)
    engine.send_did("READ", "设备运行状态信息统计E019", E019设备信息项="延时关闭时间毫秒")
    engine.expect_did("READ", "设备运行状态信息统计E019", E019设备信息项="延时关闭时间毫秒", 时间=60)
