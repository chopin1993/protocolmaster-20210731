...
def test_添加上报():
    ...
    engine.add_doc_info("4、测试添加网关后，添加上报前，是否可以正常被控制"
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('此设备为按键面板，入网无状态上报信息-0')  # 增加入网上报检测项

    engine.wait(125, allowed_message=False)
    engine.send_did("WRITE", "退网通知060B", 退网设备=config["测试设备地址"])

    engine.add_doc_info("5、测试添加上报重试的过程中，是否可以正常被控制"
                        "控制正常，被控制通断后，本产品为微电子方案，添加上报仍继续")
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('此设备为按键面板，入网无状态上报信息-1')
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.send_did("WRITE", "读写面板默认背光亮度百分比C135", '01 02')
        engine.expect_did("WRITE", "读写面板默认背光亮度百分比C135", "01 02")
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('此设备为按键面板，入网无状态上报信息-2')
        pass

    engine.wait(125, tips='等待是否有未知通讯报文')
...
def test_上电上报():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('按键面板无上电上报，不进行此项！！！')
...
def test_配置订阅者():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('测试说明：此设备是触摸面板，无配置订阅者测试项！！！')
...
def test_订阅者上报顺序测试():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('测试说明：此设备是触摸面板，无订阅者上报顺序测试项！！！')
...
def test_默认参数同时上报设备和网关():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('测试说明：此设备是触摸面板，无默认参数同时上报设备和网关测试项！！！')
...
def test_不上报():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('测试说明：此设备是触摸面板，无不上报网（不再支持D005数据标识）测试项！！！')
...
def test_只上报网关():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('测试说明：此设备是触摸面板，无只上报网关（不再支持D005数据标识）测试项！！！')
...
def test_只上报设备():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('测试说明：此设备是触摸面板，无只上报设备（不再支持D005数据标识）测试项！！！')
...
def test_广播报文控制测试():
    ...
    if config["被测设备类型"] == "开关":
        ...
        engine.add_doc_info('触摸面板类设备，手动辅助测试广播控制报文')
