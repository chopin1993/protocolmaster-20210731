...
def test_频繁通断电测试():
    ...
    def read_and_control():
        ...
        engine.send_did("READ", "继电器上电状态C060", taid=aid)
        engine.expect_did("READ", "继电器上电状态C060", "02", said=aid)
        engine.send_did("READ", "主动上报使能标志D005", taid=aid)
        engine.expect_did("READ", "主动上报使能标志D005", "00 03", said=aid)

        engine.add_doc_info("2个通道同时控制通断测试")
        engine.send_did("WRITE", "通断操作C012", "83", taid=aid)
        engine.expect_did("WRITE", "通断操作C012", "03", said=aid)
        engine.wait(1)
        engine.send_did("READ", "通断操作C012", "", taid=aid)
        engine.expect_did("READ", "通断操作C012", "03", said=aid)
        engine.wait(1)

        engine.send_did("WRITE", "通断操作C012", "03", taid=aid)
        engine.expect_did("WRITE", "通断操作C012", "00", said=aid)
        engine.wait(1)
        engine.send_did("READ", "通断操作C012", "", taid=aid)
        engine.expect_did("READ", "通断操作C012", "00", said=aid)
        engine.wait(1)
    ...
    engine.add_doc_info('设置抄控器本地密钥，默认为0，可根据实际网关PANID设置。')
