...
def test_出厂默认参数():
    ...
    3、继电器过零保护出厂为1路，可分别设置，默认断开延时5.7ms，默认闭合延时5.0ms  继电器过零点动作延迟时间C020
...
def test_通断操作C012():
    ...
    engine.relay_output_test(did="通断操作C012", relay_channel=1, output_channel=[0])
...
def test_继电器翻转C018():
    ...
    engine.relay_output_test(did="继电器翻转C018", relay_channel=1, output_channel=[0])
