...
def test_频繁通断电测试():
    ...
    def read_and_control():
        ...
        engine.expect_did("READ", "主动上报使能标志D005", "00 01", said=aid)

        engine.add_doc_info("2个通道同时控制通断测试")
        engine.send_did("WRITE", "通断操作C012", "81", taid=aid)
        engine.expect_did("WRITE", "通断操作C012", "01", said=aid)
        engine.wait(1)
        engine.send_did("READ", "通断操作C012", "", taid=aid)
        engine.expect_did("READ", "通断操作C012", "01", said=aid)
        engine.wait(1)

        engine.send_did("WRITE", "通断操作C012", "01", taid=aid)
